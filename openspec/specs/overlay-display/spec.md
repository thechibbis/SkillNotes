...
Rendering a rotation as a movable, lockable on-screen overlay of icons for the SkillNotes addon. Display-only: no click-to-cast, no combat-data reads, no per-frame API calls. Icons render from cached FileIDs.

## Requirements

### Requirement: Overlay spawning
A player can spawn a draggable overlay bound to a rotation via a slash command or the config panel's "Place Overlay" button.

#### Scenario: Spawn an overlay
- **WHEN** the player runs `/sn place <rotationName>` or clicks "Place Overlay" in the config panel
...
A player can delete an overlay via its context menu or the `/sn remove` slash command.
...
#### Scenario: Remove all overlays
- **WHEN** the player runs `/sn remove`
- **THEN** all overlays for the current character are despawned and their per-character entries removed from saved variables
